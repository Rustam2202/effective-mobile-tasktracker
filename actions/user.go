...
func CreateUser(c buffalo.Context) error {
	...
	if !ok {
		log.Logger.Error().Msg("Failed to pop connection")
	...
	if len(splitNumber) != 2 {
		...
		return c.Render(http.StatusBadRequest, r.String("Invalid passport number"))
	}
	passportSerie, err = strconv.Atoi(splitNumber[0])
	if err != nil {
		log.Logger.Error().Msg("failed parsing passportSerie")
		return c.Render(http.StatusBadRequest, r.String("Invalid passport serie"))
	}
	...
	if err != nil {
		log.Logger.Error().Err(err).Msg("Failed to get INFO_URL from .env")
		return c.Render(http.StatusInternalServerError, r.String("Internal server error"))
	}
	...
	if err != nil {
		log.Logger.Error().Err(err).Msg("Failed to get user from external server")
...
func GetAllUsers(c buffalo.Context) error {
	...
	if c.Param("id") != "" {
		...
		if err != nil {
			log.Logger.Error().Err(err).Msg("Invalid id")
	...
	if c.Param("passportSerie") != "" {
		...
		if err != nil {
			log.Logger.Error().Err(err).Msg("failed parsing passportSerie")
	...
	if c.Param("passportNumber") != "" {
		...
		if err != nil {
			log.Logger.Error().Err(err).Msg("failed parsing passportNumber")
	...
	if c.Param("page") == "" {
		...
		if err != nil {
			log.Logger.Error().Err(err).Msg("failed to parse page number")
	...
	if c.Param("per_page") == "" {
		...
		if err != nil {
			log.Logger.Error().Err(err).Msg("failed to parse perPage number")
...
func UpdateUser(c buffalo.Context) error {
	...
	if userReq.Name != "" {
		...
		log.Logger.Debug().Msg("Name to update")
	}
	if userReq.Surname != "" {
		user.Surname = userReq.Surname
		log.Logger.Debug().Msg("Surname to update")
	}
	if userReq.Patronymic != "" {
		user.Patronymic = userReq.Patronymic
		log.Logger.Debug().Msg("Patronymic to update")
	}
	if userReq.Address != "" {
		user.Address = userReq.Address
		log.Logger.Debug().Msg("Address to update")
	}
	if userReq.PassportSerie != 0 {
		user.PassportSerie = userReq.PassportSerie
		log.Logger.Debug().Msg("Passport serie to update")
	}
	if userReq.PassportNumber != 0 {
		user.PassportNumber = userReq.PassportNumber
		log.Logger.Debug().Msg("Passport number to update")
	}
